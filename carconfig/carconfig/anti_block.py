# 汽车之家反爬虫破解程序, 博客文章 http://www.cnblogs.com/dyfblog/p/6753251.html
# github: https://raw.githubusercontent.com/duanyifei/antispider/master/autohome.py
import re
import requests
import traceback
...
if __name__ == '__main__':
    ...
    # # 论坛
    # resp = requests.get("http://club.autohome.com.cn/bbs/threadqa-c-3170-64250499-1.html")
    ...
    # 口碑
    resp = requests.get(
        # "http://k.autohome.com.cn/spec/25829/view_1593573_1.html",
        'http://k.autohome.com.cn/spec/193/ge1/0-0-2/#dataList')
    text = get_complete_text_autohome(resp.text)
    # text = re.sub("<style[^>]+?>[\s\S]+?</style>", "", text)
    # text = re.sub("<script[^>]?>[\s\S]+?</script>", "", text)
    # text = re.search("<div\s*class=[\'\"]text-con[^\'\"]*?[\'\"]>([\s\S]+?)</div>", text).group(1)

    # print(text)

    # f = open('real_source.html', 'w', encoding='utf8')
    # f.write(text)
    # f.close()
    ...
    modified_sel = Selector(text=text, type='html')
    content_blocks = modified_sel.xpath('//div[@class="mouthcon"]')

    for content_block in content_blocks:
        content = content_block.xpath('.//div[@class="text-con"]/text()').extract()
        content = '\r'.join(content).strip()
        review_content = re.sub('【.*】', '', content)
